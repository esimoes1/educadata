## Enem_por_escola.R - Produz os resultados do ENEM 2016
...
# Carrega dados ####
load("microdados_enem_2016_escola.RData")

# Sinaliza informações sobre a particpação dos alunos
enem2016[is.na(enem2016$TP_PRESENCA_CH),]$TP_PRESENCA_CH <- 0
enem2016[is.na(enem2016$TP_PRESENCA_CN),]$TP_PRESENCA_CN <- 0
enem2016$fl_inscrito <- 1
enem2016$fl_ausente_dia1 <- ifelse(enem2016$TP_PRESENCA_CH==0,1,0)
enem2016$fl_ausente_dia2 <- ifelse(enem2016$TP_PRESENCA_CN==0,1,0)
enem2016$fl_participante <- ifelse(enem2016$TP_PRESENCA_CN==1 & enem2016$TP_PRESENCA_CH==1,1,0)
enem2016$fl_anulada <- ifelse(enem2016$fl_participante==0 & (enem2016$TP_PRESENCA_CH==2 | enem2016$TP_PRESENCA_CN==2),1,0)
...
censo_matriculas <- setNames(dcast(subset(turmas2016,!TP_ETAPA_ENSINO %in% c(29,34)),CO_ENTIDADE~"NU_MATRICULAS",value.var="NU_MATRICULAS",sum),c("id_escola","nr_matriculas"))
...
enem_result <- subset(enem2016,CO_ESCOLA %in% subset(enem_part,fl_valida==1)$id_escola)
...
escolas2016 <- merge(escolas2016,redes,by="TP_DEPENDENCIA")
escolas2016 <- setNames(escolas2016,c("cd_dependencia","id_escola","nm_escola","id_municipio","ds_dependencia"))
...
enem_escola <- merge(enem_escola,escolas2016,by="id_escola")
...
write.csv2(enem_escola,"enem_por_escola_2016.csv",row.names=F,na="")
